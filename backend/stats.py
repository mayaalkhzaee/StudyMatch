...
async def get_stats(request: Request):
    ...
    top_courses = [{"course": k, "session_count": v} for k, v in sorted_courses[:5]]
    
    sorted_dates = sorted(date_counts.items(), key=lambda item: item[0])
    sessions_per_week = [{"week": k, "count": v} for k, v in sorted_dates]
    ...
    top_participants = [{"username": k, "joined_count": v} for k, v in sorted_users[:5]]
